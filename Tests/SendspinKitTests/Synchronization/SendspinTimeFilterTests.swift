...
@Suite("SendspinTimeFilter")
struct SendspinTimeFilterTests {

    // MARK: - Initialization

    @Test("fresh filter has uninformative prior")
    func freshFilter() {
    ...
    @Test("first measurement initializes offset")
    func firstMeasurement() {
        var filter = SendspinTimeFilter()
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 50.0)

        #expect(filter.isInitialized)
        #expect(filter.count == 1)
        #expect(filter.offset == 500.0)
        #expect(filter.drift == 0.0)
        #expect(filter.offsetCovariance == 2500.0) // 50²
    }

    @Test("second measurement initializes drift")
    func secondMeasurement() {
        var filter = SendspinTimeFilter()
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 50.0)
        filter.update(timeAdded: 2000, measurement: 510.0, maxError: 50.0)
    ...
    @Test("converges on constant offset with zero drift")
    func convergesConstantOffset() {
        var filter = SendspinTimeFilter()
        let trueOffset = 1000.0
        ...
        #expect(filter.offsetCovariance < 2500.0) // well below initial prior
    }

    @Test("tracks linear drift")
    func tracksLinearDrift() {
        var filter = SendspinTimeFilter()
        let trueOffset: Double = 1000.0
        let trueDrift: Double = 0.001 // 1μs per ms = 1ppm
    ...
    @Test("handles zero maxError (localhost)")
    func zeroMaxError() {
        var filter = SendspinTimeFilter()
        // maxError of 0 should be floored to 1.0 internally
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 0.0)
    ...
    @Test("no NaN after many localhost updates")
    func noNaNOnLocalhost() {
        var filter = SendspinTimeFilter()
    ...
    func rejectsNonMonotonic() {
        var filter = SendspinTimeFilter()
        filter.update(timeAdded: 2000, measurement: 500.0, maxError: 50.0)
        filter.update(timeAdded: 1000, measurement: 600.0, maxError: 50.0) // earlier!

        #expect(filter.count == 1)
        #expect(filter.offset == 500.0) // unchanged
    }

    @Test("rejects duplicate timestamp")
    func rejectsDuplicateTimestamp() {
        var filter = SendspinTimeFilter()
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 50.0)
        filter.update(timeAdded: 1000, measurement: 600.0, maxError: 50.0) // same time
    ...
    @Test("drift not used when statistically insignificant")
    func driftSignificanceGate() {
        var filter = SendspinTimeFilter()
        // Two measurements close together → noisy drift estimate → should not use drift
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 50.0)
        filter.update(timeAdded: 1100, measurement: 500.5, maxError: 50.0)
    ...
    @Test("computeServerTime and computeClientTime are inverses")
    func timeConversionRoundTrip() {
        var filter = SendspinTimeFilter()
        let trueOffset = 5000.0
    ...
    @Test("computeServerTime applies offset correctly")
    func computeServerTimeOffset() {
        var filter = SendspinTimeFilter()

        // Establish a known offset of 1000μs
        for i in 0..<10 {
            let t = Int64((i + 1) * 100_000)
            filter.update(timeAdded: t, measurement: 1000.0, maxError: 10.0)
        }
        ...
        #expect(abs(serverTime - (clientTime + 1000)) < 5)
    }

    // MARK: - Reset

    @Test("reset returns to uninformative prior")
    func resetFilter() {
        var filter = SendspinTimeFilter()
        filter.update(timeAdded: 1000, measurement: 500.0, maxError: 50.0)
        filter.update(timeAdded: 2000, measurement: 510.0, maxError: 50.0)
    ...
    @Test("covariance shrinks with consistent measurements")
    func covarianceShrinks() {
        var filter = SendspinTimeFilter()

        var lastCovariance: Double = .infinity
        for i in 0..<30 {
            let t = Int64((i + 1) * 100_000)
            filter.update(timeAdded: t, measurement: 1000.0, maxError: 50.0)
            ...
            if i > 2 {
                ...
                #expect(filter.offsetCovariance < lastCovariance * 1.5,
        ...
        #expect(filter.offsetCovariance < 2600.0)
    }

    // MARK: - Estimated error

    @Test("offset covariance decreases with consistent measurements")
    func covarianceDecreases() {
        ...
        filter.update(timeAdded: 100_000, measurement: 1000.0, maxError: 50.0)
