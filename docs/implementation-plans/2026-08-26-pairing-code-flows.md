...
| C.4 | Static dynamic-only fields and receiving `server/pair-init` during a static attempt are protocol errors with silent close; a `format` aborts with `method_not_supported`; unwrapped finalize is a protocol error with silent close and no persistence | `StaticPairingProtocolErrorTests` |
